...
export const bringAllCharacters = async () => {
  const res = await axios.get(`${API_URL}/professors` /*headers*/);
  const [professors,count] = res.data
  return professors;
};
...
export const bringAppointmentsById = async (token) => {
  ...
  const res = await axios.get(`${API_URL}appointments/student/appointments/`, config);

  return res.data;
};

export const bringProfessorsAppointmentsById = async (token) => {
  // puedo preparar la información para enviar al servidor
  const config = {
    headers: {
      Authorization: `Bearer ${token}`
  ...
  const res = await axios.get(`${API_URL}appointments/professor/appointments/`, config);
